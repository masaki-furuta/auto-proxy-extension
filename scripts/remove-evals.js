...
const BUNDLE_DIR = path.resolve(path.join(__dirname, '../dist'))
const bundles = ['background.js', 'options.js', 'popup.js']
...
const evalRegexForDevelopment = /;([a-z])=function\(\){return this}\(\);try{\1=\1\|\|Function\("return this"\)\(\)\|\|\(0,eval\)\("this"\)}catch\(t\){"object"==typeof window&&\(\1=window\)}/g
...
const main = () => {
  bundles.forEach(bundle => {
    removeEvals(path.join(BUNDLE_DIR, bundle))
      .then(() => console.log(`Bundle ${bundle}: OK`))
      .catch(console.error)
  })
}
